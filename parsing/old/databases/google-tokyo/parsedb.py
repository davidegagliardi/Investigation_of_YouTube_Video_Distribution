...
with open(filename,'w') as csvfile:
    ...
    for results in sorted(glob.glob('*.db')):
        ...
        try:
            ...
            if not rows:
                print("Empty")
                line = ["Asia/Google","E","null","null","null","null","null","null","null","null","null"]
                print(line)
                csvwriter.writerow(line)
            else:
                for row in rows:
                    line = ["Asia/Google","E",row[0],row[2],row[3],row[5],row[8],row[10],row[25],row[27],row[28]]
                    print(line)
                    csvwriter.writerow(line)
            conn.close()
